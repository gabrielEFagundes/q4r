# Voidstar — VS Code syntax highlighting
...
4. In that new window, open any `.st` or `.star` file

Changes to `syntaxes/voidstar.tmLanguage.json` apply after reloading the dev host
...
- `syntaxes/voidstar.tmLanguage.json` → all coloring logic. Each `repository`
